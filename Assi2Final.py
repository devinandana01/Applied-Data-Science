...
def data_f(a):
    ...
    Returns :     
        datafile (pd.DataFrame): the pandas DataFrame that contains the data 
        from the EXCEL file
...
#Reading the file to the dataframe 'data_file'
...
def stat_f(c):
    """
    This function takes a pandas DataFrame `c` and performs several 
    statistical calculations on the columns.
    It prints the summary statistics, correlation matrix, skewness, 
    and kurtosis
    ...
    print(skew(stats, axis=0, bias=True), kurtosis(stats, axis=0, bias=True), 
          stats.describe(), stats.cov())
...
def elec_cons(d):
    """
        ...
        This function takes a pandas DataFrame `d` containing data on worldbank
        climate change data and creates a barplot 
        of the percentage change in Electric power consumption for a selection 
        of countries.
    ----------
    ...
    # Select rows where the "Indicator Name" column is "Electric power
    ...
    elec = data1[data1['Country Name'].isin(['Australia','Canada','Germany',
    ...
    plt.bar(r1, elec['1990'], color='lime', width=bar_width, 
            edgecolor='black', label='1990')
    plt.bar(r2, elec['1995'], color='olive', width=bar_width,
            edgecolor='black', label='1995')
    plt.bar(r3, elec['2000'], color='mediumaquamarine', width=bar_width, 
            edgecolor='black', label='2000')
    plt.bar(r4, elec['2005'], color='darkgreen', width=bar_width, 
           
            edgecolor='black', label='2005')
    plt.bar(r5, elec['2010'], color='yellowgreen', width=bar_width,
            edgecolor='black', label='2010')

    # Set the x-tick labels to the country names
    plt.xticks([r + bar_width*2 for r in range(len(elec))],
               elec['Country Name'], fontsize=15)
...
def elec_prod(e):
    ...
    This function takes a pandas DataFrame `e` containing data on worldbank 
    climate change data and creates a bar
    plot of the percentage change in Electricity production from renewable 
    sources for a selection of countries.
    ----------
    ...
    # Select rows where the "Country Name" column is one of a list of countries
    el = dt1[dt1['Country Name'].isin(['Australia','Canada','Germany',
                                       'France','Ireland','Japan','Poland',
                                       'Sweden'])]
    ...
    # Create a bar plot of the selected data, with a different color for each year
    plt.subplots(figsize=(10, 8))
    plt.bar(r1, el['1990'], color='peru', width=bar_width, 
            edgecolor='black', label='1990')
    plt.bar(r2, el['1995'], color='saddlebrown', width=bar_width,
            edgecolor='black', label='1995')
    plt.bar(r3, el['2000'], color='coral', width=bar_width, 
            edgecolor='black', label='2000')
    plt.bar(r4, el['2005'], color='indianred', width=bar_width, 
            edgecolor='black', label='2005')
    plt.bar(r5, el['2010'], color='maroon', width=bar_width,
            edgecolor='black', label='2010')

    # Set the x-tick labels to the country names
    plt.xticks([r + bar_width*2 for r in range(len(el))], el['Country Name'], 
               fontsize=15)
...
def dt_lineplot(f):
    ...
    Plots a line graph of Urban population growth (annual %) of selected
    countries from the given dataframe
    ...
    data2 = con[con['Country Name'].isin(['Australia','Canada','Germany',
                                          'France','Ireland','Japan','Poland',
                                          'Sweden'])]
    ...
    plt.plot(data_transpose.index, data_transpose['Australia'], 
             linestyle='dashed', label='Australia')
    plt.plot(data_transpose.index, data_transpose['Canada'],
             linestyle='dashed', label='Canada')
    plt.plot(data_transpose.index, data_transpose['Germany'],
             linestyle='dashed', label='Germany')
    plt.plot(data_transpose.index, data_transpose['France'], 
             linestyle='dashed', label='France')
    plt.plot(data_transpose.index, data_transpose['Ireland'],
             linestyle='dashed', label='Ireland')
    plt.plot(data_transpose.index, data_transpose['Japan'], 
             linestyle='dashed', label='Japan')
    plt.plot(data_transpose.index, data_transpose['Poland'],
             linestyle='dashed', label='Poland')
    plt.plot(data_transpose.index, data_transpose['Sweden'], 
             linestyle='dashed', label='Sweden')
...
def elec_lineplot(g):
    """
        ...
        This function takes a pandas dataframe 'g' as input and plots a line
        graph showing the Renewable electricity
        output (% of total electricity output) for 8 different countries.
    ----------
    ...
    renew_elec = data3[data3['Country Name'].isin(['Australia','Canada',
    ...
    #plotting the line graph
    plt.figure(figsize=(10,8))
    plt.plot(data3_transpose.index, data3_transpose['Australia'],
             linestyle='dashed', label='Australia')
    plt.plot(data3_transpose.index, data3_transpose['Canada'], 
             linestyle='dashed', label='Canada')
    plt.plot(data3_transpose.index, data3_transpose['Germany'], 
             linestyle='dashed', label='Germany')
    plt.plot(data3_transpose.index, data3_transpose['France'], 
             linestyle='dashed', label='France')
    plt.plot(data3_transpose.index, data3_transpose['Ireland'],
             linestyle='dashed', label='Ireland')
    plt.plot(data3_transpose.index, data3_transpose['Japan'],
             linestyle='dashed', label='Japan')
    plt.plot(data3_transpose.index, data3_transpose['Poland'],
             linestyle='dashed', label='Poland')
    plt.plot(data3_transpose.index, data3_transpose['Sweden'], 
             linestyle='dashed', label='Sweden')
...
def heatmap_ireland(h):
    ...
    different indicators for Ireland.

    Args:
    h (pandas.DataFrame): A DataFrame containing data on different 
    ...
    sns.heatmap(ireland_df.corr(),fmt='.2g', annot=True, cmap='Greens', 
                linecolor='black')
    plt.xlabel('')
    plt.ylabel('')
...
def heatmap_canada(i):
    """
        ...
        A function that creates a heatmap of the correlation matrix between
        different indicators for Canada.

    Args:
    x (pandas.DataFrame): A DataFrame containing data on different indicators
    ...
    # Plot the heatmap using seaborn
    plt.figure(figsize=(12,8))
    sns.heatmap(Canada_df.corr(), fmt='.2g', annot=True, cmap='BuPu',
                linecolor='black')
